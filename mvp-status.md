...
Status: Product MVP not yet end-user usable
Date: 26 June 2026
...
The previous implementation and evidence prove SSH-backed primitives and a command-driven smoke path. They do not yet prove an end-user-usable terminal IDE. The usable MVP boundary is now defined by:

- [terminal-ide-style-guide.md](terminal-ide-style-guide.md)
- [terminal-ide-user-stories.md](terminal-ide-user-stories.md)

A Product MVP cannot be called complete until the prioritized MVP user stories in that document pass with end-user style evidence.
...
## Confirmed technical smoke scope

The existing smoke path confirms:
...
| Render terminal workbench | Static workbench frame with Explorer/editor/panel/status is captured under `test-evidence/product-mvp/frames/`. |
| Manual command flow | `npm run smith -- ide-demo` lets a tester type commands to browse, open, edit, save, search, run a command, and quit. |
...
These gaps must be closed before Product MVP is complete:

- real interactive raw-mode TUI event loop;
- visible focus and mode model;
- navigable Explorer controlled by keyboard and mouse;
- editor interaction using visible Normal/Insert modes;
- command palette and context help;
- minibuffer prompts for search, command entry, confirmations, and errors;
- integrated remote terminal panel with visible terminal-mode escape;
- resize behavior evidenced across wide, medium, narrow, short, and minimum layouts;
- dirty-buffer protection on exit/disconnect/save failure;
- end-user evidence screenshots for all MVP user stories.
